...
def scan_file(reader, file_path):
    ...
    ocr_result = reader.readtext(
        image,
        width_ths=config["scan"]["width_ths"],
        x_ths=config["scan"]["x_ths"],
        batch_size=400,
    )
...
def match_card(card_set, chunk):
    ...
    for ocr_result in chunk:
        ...
        if highest_ratio >= 85:
            ...
            print(
                f'Passed: {highest_ratio}, {ocr_result[1]}, {highest_card.get("card").get("foil")}, {ocr_result[3]}'
            )  # type: ignore
        else:
...
def match(card_db, ocr_db):
    card_set = [
        {
            ...
            or card.get("prices").get("usd_foil"),
            "foil": card.get("foil"),
        }
        for card in card_db
    ]
    ocr_cursor = ocr_db.cursor()
    ocr_cursor.execute("SELECT id, file_name, ocr_result, type FROM cards;")
    ...
    ocr_result_chunks = [
        ocr_db_all[i : i + chunk_size]
        #  for i in range(0, 1, 1)]  # type: ignore
        for i in range(0, len(ocr_db_all), chunk_size)
    ]  # type: ignore
    with concurrent.futures.ProcessPoolExecutor(max_workers=16) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            for passed_card in passed_cards:
                ocr_db.cursor().execute(
                    "INSERT INTO match_results (ocr_id,name,ocr_result,price,foil,ratio) VALUES (%s,%s,%s,%s,%s,%s)",
                    (
                        passed_card.get("id"),
                        passed_card.get("card").get("name"),
                        passed_card.get("ocr"),
                        passed_card.get("card").get("price"),
                        passed_card.get("card").get("foil"),
                        passed_card.get("ratio"),
                    ),
                )
                ocr_db.commit()
            for failed_card in failed_cards:
                ocr_db.cursor().execute(
                    "INSERT INTO failed_results (ocr_id,name,ocr_result,price,foil,ratio) VALUES (%s,%s,%s,%s,%s,%s)",
                    (
                        failed_card.get("id"),
                        failed_card.get("card").get("name"),
                        f'{failed_card.get("ocr")}',
                        failed_card.get("card").get("price"),
                        f'{failed_card.get("card").get("foil")}',
                        failed_card.get("ratio"),
                    ),
                )
                ocr_db.commit()
